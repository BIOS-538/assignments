...
editor: 
  visual:
    embed-resource: true # +1 bonus point for explaining why we have this right here!
---
...
This exam is meant to test these topics you have learnt in the last two months while serving as practice to brush up these concepts. We anticipate this to be at the same complexity/time commitment as a homework assignment, **more weightage** will be given for providing short explanations(and **documentation** in code) that **demonstrate that you understand** what the R code you wrote does.

Please remember that **there are many ways of answering the questions**, and all approaches are valid for grading. *If you tried and couldn't answer any question, partial points will be given if you add enough explanation to indicate that you are on the right path.* If you are having trouble understanding or interpreting any question, please do reach out to the TAs/discuss with peers/post on the discussion board.
...
-   Making informative plots with proper axis labels, titles, and annotations is handy to communicate well.

-   Minor concepts

    -   Why does assignment hide the result of the calculation; show two contexts where you see this (within other functions)
    -   Subsetting a\[i\] vs a\[\[i\]\]
    -   ggplot - where to write the aes (`ggplot` vs `geom_..` ) and what to write inside of the `aes()`
    -   (extra) Get people to generate and plot a normal distribution along with mean/SD?
    -   Q4 \~ replace with a galton like simulation (of calculating means)
    -   Q4 : bonus \~ show t.test results from 3 comparison samples with different expected results
    -   Q5 \~ Figure transform to something better // vs re-create a figure from a paper (probably won't be as good?)

## Resources

Use this [section](https://r4ds.hadley.nz/quarto.html#visual-editor) for a refresher if you are having trouble navigating the quarto file, or doing actions making a code chunk etc.

## Points distribution

|       | Q0  | Q1  | Q2  | Q3  | Q4  | Q5  | Q6  | Q7  | Q8  | sum |
|-------|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|
| Reg   | \-  | 9   | 7   | 9   | 6   | 10  | 6   | 3   | 5   | 55  |
| Bonus | 1+2 | 1   |     |     |     | 2   | 2   | 3   | 1   | 12  |
...
## 1. Check your understanding [(9+1 point)]{style="color: red;"}

(\~ [1 point]{style="color: red;"} for each question + 1 bonus)
...
### 1d.bonus. working directory vs quarto [(+1 point)]{style="color: red;"}
...
## 4. Troubleshooting [(6 *points,*]{style="color: red;"} 2 per question[)]{style="color: red;"}
...
## 5. Make a better ggplot [(10 points)]{style="color: red;"}
...
sldat <- arrange(sleeping_data, sleep_total) %>% 
  mutate(order = fct_inorder(order)) %>% 
  group_by(order) %>% mutate(counts = n())

# re-formatted plot
ggplot(sldat, aes(sleep_total, order)) + 
         
  geom_boxplot() + 
  geom_point(alpha = 0.3, position = position_jitter(height = 0.2)) + 
  
  # add text
  geom_text(aes(label = counts, x = 0)) + 
  
  # axis labels
  labs(title = 'Mammals **_sleep duration_** changes across **_orders_** with a wide variation', 
       subtitle = 'Number of members in each order specified next to the y-axis',
       x = 'Sleep duration per day (hours)',
       y = NULL) + 
...
ggsave('../../outputs/sleep_barplot.png', width = 6, height = 5)
```

Find the target plot that you want to work towards as an attachment on canvas `sleep_barplot.png`

### 5a. Discuss visual choices [(4 points)]{style="color: red;"}

Contrast the output of the initial barplot with the desired final plot and make 3 brief observations to answer - In what aspects is latter better than the former?

![](https://imgur.com/MT92oYy)

![](../../outputs/sleep_barplot.png)

### 5b. Improve the plot [(6 points)]{style="color: red;"}
...
2.  Overlay points on the boxplot with some transparency [(1 point)]{style="color: red;"}
    1.  Add some noise to the points using `position_jitter()`, [(1 point)]{style="color: red;"}
...
### 5b.bonus. Rearrange data order [(+1 point)]{style="color: red;"}
...
### 5c.bonus : format title text [(+1 point)]{style="color: red;"}
