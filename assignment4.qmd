...
-   Total = 14.25 points + 6.25 bonus. *Most questions are 0.5 point each but Q5-8 have the bulk of the points* 😉
...
**Make sure to change the filename to include your first name as a suffix `assignment4_.qmd`**
...
(*Prelim rubric*) '\[0.25 points for each histogram\] ; '\[-0.25 if they don't explain the pattern they see\]

## Q2: Scatter plot

You now wish to explore the relationship between poverty and college degrees. To do this, we create a scatterplot of `percbelowpoverty` vs. `percollege`. Color the data-points by the `inmetro` column *(0.5 points)*

You will need to add a new column that is `as.factor()` of the `inmetro` column to the dataset and write what a 0 means and what a 1 means).
...
(***Bonus 2.a.**/+0.25 points)* Why do we need to use `as.factor()` above for the `inmetro` column? Plot what happens if you don't use this and answer this question

```{r why-factor}

```

(***Bonus 2.b/**+0.25 points)* Can you modify the factor version of the inmetro column's levels to be more informative than 0 and 1?
...
Find the mean for EACH STATE and repeat the above question, with rhe above_mean_density specific to the state the county is found in. (*1 point)*

*Hint:* need to use `group_by()`

(*Prelim rubric*) : '\[-0.25 if calculate the mean incorrectly\] ; '\[-0.25 if they do not color by state\] ; '\[-0.5 if barplot is incorrect\]
...
Investigate the average life expectancy for the most recent year for each continent. Make a table to show this data (*0.5 point*)
...
Can you rearrange the rows to emphasize the data of countries with the least health outcomes in the latest year? To answer this, arrange the data in descending order of year and ascending order of life expectancy (*0.5 point)*
